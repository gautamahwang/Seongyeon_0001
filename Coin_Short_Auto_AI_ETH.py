...
logging.basicConfig(filename='./error_log_ETH.log',level=logging.ERROR)
...
def predict_price(ticker):
    ...
    # 시간 봉, 아무 조건 없을 경우 최근 200개의 데이터를 가져옴
...
# 시작 메세지 슬랙 전송
post_message(myToken,"#upbit_auto", "auto trade has been started_ETH")

# 자동매매 시작
krw_total = get_balance("KRW")
fixed_k = float(get_new_k())
ordered_price = {'KRW-ETH':0}
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            target_price = get_target_price("KRW-ETH", fixed_k)
            print('target_price')
            print(target_price)
            ...
            #상승조건 매도
            if 0 < ordered_price['KRW-ETH'] * 1.15 < current_price :
                upbit.sell_market_order("KRW-ETH", coin_balance*0.9995)
                post_message(myToken,"#upbit_auto", "KRW-ETH " + " sold " )
                krw_total = get_balance("KRW")
            #하강조건 매도
            if current_price < ordered_price['KRW-ETH'] * 0.80 and current_price < predicted_close_price:
                upbit.sell_market_order("KRW-ETH", coin_balance*0.9995)
                post_message(myToken,"#upbit_auto", "KRW-ETH " + " sold " )
                krw_total = get_balance("KRW")
        ...
        #매도
            ...
            coin_balance = get_balance("ETH")
            current_price = get_current_price("KRW-ETH")
            fixed_k = float (get_new_k())
            post_message(myToken,"#upbit_auto", 'ETH')
            post_message(myToken,"#upbit_auto", fixed_k)
        ...
        post_message(myToken,'#upbit_auto', 'ETH Error')
