        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            if target_price < current_price and current_price < predicted_close_price:
                ...
                if krw > 5000: #최소 거래 금액
                    ...
                    print(buy_result)
                    post_message(myToken,"#bit_auto_", str("KRW-KNC") + " buy : " +str(buy_result) + '/ Balance : '+ str(krw))
            #매도
            else:
                coin_balance = get_balance("KRW-KNC")
                current_price = get_current_price("KRW-KNC")
                print(coin_balance)
                print(current_price)
                ...
                if coin_balance > 5500 / current_price: #최소 거래금액 5,000원 이므로
                    print('여기2')
                    sell_result = upbit.sell_market_order("KRW-KNC", coin_balance*0.9995)
                    print(sell_result)
                    post_message(myToken,"#bit_auto_", "KRW-KNC " + " sell : " +str(sell_result) + '/ Balance : ' + str(krw))
            time.sleep(1)
